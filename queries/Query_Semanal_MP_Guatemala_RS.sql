select a.fecha, a.total, b.total2 from (    
	...
	as fecha, count(averia.averia_id) as total
...
select a.fecha, a.total, b.total2 from (    
	...
	where tipo_rep_id=2 and proceso_id=7 AND informante_id=3 and fecho_detec != 0 and 
	((weekofyear(fecho_detec) >= weekofyear('2013-10-01') 
	and weekofyear(fecho_detec) <= weekofyear(DATE_ADD('2013-10-01',INTERVAL  31 day)) and fecho_detec != 0) or
	(weekofyear(fecho_repar) >= weekofyear('2013-10-01') 
	and  weekofyear(fecho_repar) <= weekofyear(DATE_ADD('2013-10-01',INTERVAL  31 day)) and fecho_detec = 0) or
	(weekofyear(fecho_repar) >= weekofyear('2013-10-01') 
	and  weekofyear(fecho_repar) <= weekofyear(DATE_ADD('2013-10-01',INTERVAL  31 day)) 
	and fecho_detec != 0 and fecho_repar != 0))
	GROUP by fecha
	)as a left join (                 
	-- mantenimientos ejecutados
	select if( 
	if(weekofyear(fecho_detec),weekofyear(fecho_detec),0) >= if(weekofyear(fecho_repar),weekofyear(fecho_repar),0),
	if(weekofyear(fecho_repar), weekofyear(fecho_repar), weekofyear(fecho_detec)),
	if(weekofyear(fecho_detec), weekofyear(fecho_detec), weekofyear(fecho_repar))) as fecha,
