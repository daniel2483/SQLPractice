...
CREATE DEFINER=`root`@`%` PROCEDURE `Insert_equipo_rur`()
BEGIN
-- Procedure para Cargar Datos de la Tabla tem_claro_cr.sitios_rural a la tabla 
-- claro_cr.equipo
	...
	DECLARE cur_ CURSOR FOR select 	sitios_rural_id,
									nemonico_id,
									Tipo_sitio,
									Placa,
									Nombre_Activo,
									...
									No_localizacion,
									Observaciones,
									Cargado from tem_claro_cr.sitios_rural where cargado = 0;
	...
	FETCH cur_ into temp_sitio_id,
      ...
      rep_open_loop:REPEAT
		...
		-- select Nombre_de_Activo,Area,Cantidad,Banda,Marca,Modelo,Serie,Configuracion,Sectores into temp_nombre_act,temp_area,temp_cantidad,temp_banda,temp_marca,temp_modelo,temp_serie,temp_configuracion,temp_sectores from tem_claro_cr.sitios_rural;
...
--				temp_activo; -- temp_capacidad;
		 ...
		 IF (temp_sitio_id is not NULL AND temp_cargado=0) then
			...
			UPDATE tem_claro_cr.sitios_rural set Cargado = 1 where sitios_rural_id=temp_sitio_id;
		END IF;
