...
declare cod_proceso int default 5;
declare cod_texto int default 77;
declare tt_ref varchar(15) default 'NA';
declare cod_prioridad int default 7; #prioridad solicitud
...
declare id_tmp int;
declare complet bit;
declare cod_cierre int;



DECLARE done int;

DECLARE cur_ CURSOR FOR select idTemp_preventivos_infra, nemonico, completado , fecha from Temp_preventivos_infra where cargado <> 1;
...
select ESTACION,complet,FECHA_DETEC;
      rep_open_loop:REPEAT
		...
		update Temp_preventivos_infra set cargado =1 where id_tmp;
		end if;
