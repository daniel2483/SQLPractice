...
CREATE DEFINER=`root`@`%` PROCEDURE `Insert_equipo_reparacion`()
BEGIN

	DECLARE temp_reparacion_id int(11);
	...
	DECLARE cur_ CURSOR FOR select 	reparacion_id,
									codigo,
									descripcion,
									serie,
									proyecto,
									...
									cargado 
									from 
									tem_claro_cr.reparacion 
									where cargado = 0;
	
	DECLARE CONTINUE HANDLER FOR NOT FOUND SET done=1;
	OPEN cur_;

	FETCH cur_ into temp_reparacion_id,
					temp_codigo,
					temp_descripcion,
					temp_serie,
					temp_proyecto,
      ...
      rep_open_loop:REPEAT
		 ...
		 IF (temp_reparacion_id is not NULL AND temp_cargado=0) then
			...
			UPDATE tem_claro_cr.reparacion set cargado = 1 where reparacion_id=temp_reparacion_id;
		END IF;
